# File: app/ui/file_tree_delegate.py
...
class FileTreeDelegate(QStyledItemDelegate):
    def __init__(self, parent=None, modified_files_resolver=None, prompts_root_resolver=None):
        ...
        if self.script_icon.isNull():
            self.script_icon = QApplication.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView)
        ...
        if self.character_folder_icon.isNull():
             self.character_folder_icon = QIcon.fromTheme("folder-saved-search") # Another option
        if self.character_folder_icon.isNull():
            self.character_folder_icon = QApplication.style().standardIcon(QStyle.StandardPixmap.SP_DirHomeIcon)
        if self.character_folder_icon.isNull(): 
            self.character_folder_icon = QApplication.style().standardIcon(QStyle.StandardPixmap.SP_DirIcon)
        ...
        if file_info.isFile() and file_info.suffix().lower() == "script":
            ...
            if prompts_root_str:
                try:
                    ...
                    if current_dir_path.parent == prompts_root_path and \
                       not current_dir_path.name.startswith("_") and \
                       current_dir_path != prompts_root_path: 
                        if not self.character_folder_icon.isNull():
                            option.icon = self.character_folder_icon
                except Exception: # Path operations can fail with invalid paths
                    pass # Default icon will be used
        
        if self.modified_files_resolver:
            modified_files = self.modified_files_resolver()
            current_file_path = index.model().filePath(index) 
            if current_file_path in modified_files:
